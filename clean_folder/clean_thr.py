...
def around_dir(main_path, video_path, audio_path, images_path, documents_path, archives_path, other_path):
    ...
    for file in files:
        ...
        if os.path.isfile(file_path):
            file_handler(file, file_path, main_path, video_path, audio_path,
                         images_path, documents_path, archives_path, other_path)
        else:
            around_dir(file_path, video_path, audio_path, images_path,
                       documents_path, archives_path, other_path)
...
if __name__ == "__main__":
    before = time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_count()*2+1) as executor:
